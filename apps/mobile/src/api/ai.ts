...
export interface ParseBillResult {
  ...
  intent?: 'expense' | 'income' | 'transfer' | 'analysis' | 'chat' | 'not_bill';
  busy?: boolean;
  timeout?: boolean;
  message?: string;
}
...
export const aiApi = {
  ...
  chat(message: string, history: AiChatMessage[] = []) {
    return request<AiChatResponse & { busy?: boolean; timeout?: boolean; message?: string; intent?: string; transactions?: unknown[] }>('/ai/chat', {
      method: 'POST',
